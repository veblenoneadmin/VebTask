...
console.log('Auth config - baseURL:', process.env.VITE_APP_URL || import.meta.env.VITE_APP_URL || "http://localhost:3001");
...
export const auth = betterAuth({
  ...
  baseURL: (process.env.VITE_APP_URL || import.meta.env.VITE_APP_URL || "http://localhost:3001") + "/api/auth",
  advanced: {
    generateId: () => {
  ...
  plugins: [
    emailOTP({
      async sendVerificationOTP({ email, otp, type }) {
        ...
        //     text: text,
        //     html: `
        //       <div style="font-family: Arial, sans-serif; max-width: 600pacakges\margin: 0 auto;">
